    ...
    stages {
        ...
        stage('Env Check') {
            environment {
                ...
                APP_PROCESS_ID_THROUGH_SH = sh(script: "jps | grep `mvn help:evaluate -Dexpression=project.build.finalName | grep \"^[^\\[]\"`.`mvn help:evaluate -Dexpression=project.packaging | grep \"^[^\\[]\"` | awk \'{print \$5}\'", , returnStdout: true).trim()}
            steps {
                ...
                sh 'echo "APP_PROCESS_ID_THROUGH_SH is $APP_PROCESS_ID_THROUGH_SH"'
                sh 'printenv'
                // 打印环境变量时用单引号包裹
...
//         }
        stage('Kill') {
            ...
            when {
              allOf {
                branch 'develop'
                expression { return "`cat /opt/module/build/$BUILD_ID/app_id`" > 0}
              }
            }
            steps {
                sh 'echo "APP_PROCESS_ID is $APP_PROCESS_ID"'

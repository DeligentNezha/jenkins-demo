...
//     }
    ...
    stages {
        ...
        stage('Env Check') {
            ...
            steps {
                sh 'echo "$PPID $$ $@" > /opt/module/export-pid; pgrep -la -u admin >> /opt/module/export-pid; { EXPORT_APP_PROCESS_ID=$(jps | grep $ARTIFACT_FILENAME | awk \'{print $1}\'); echo $EXPORT_APP_PROCESS_ID; export EXPORT_APP_PROCESS_ID; pstree -ahgGls admin > /opt/module/jenkins-pipeline; }'
...
//         }
        stage('Kill') {
            ...
            when {
              allOf {
                branch 'develop'
                expression { return env.APP_PROCESS_ID > 0}
              }
            }
            steps {
                sh 'echo "APP_PROCESS_ID is $APP_PROCESS_ID"'
